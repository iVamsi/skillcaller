...
**Final Audit Status:** **Exemplary (Grade A+) — 142/142 Tests Passing, 97% Coverage, Live-Tested on Claude Code, Codex, Cursor & Antigravity**

---

## 1. Executive Summary

**`skillcaller`** is an automated trigger-reliability evaluation harness for [Agent Skills](https://agentskills.io) supporting **Claude Code**, **OpenAI Codex**, **Cursor Agent**, and **Antigravity**. It addresses the critical blind spot of prompt-trigger accuracy: **skills whose descriptions fail to activate when needed (false negatives), trigger inappropriately on unrelated queries (false positives), or collide and hijack adjacent skills.**

### Key Metrics Summary
- **Test Suite**: 20 test files, **142 tests passing**, 0 failures
- **Live Smoke Tests**: Live-tested against authenticated CLIs for Claude Code, Codex, Cursor, and Antigravity (`LIVE=1 npm run smoke`)
- **Code Coverage**: 96.95% Lines, 88.00% Branches, **100% Functions**, 96.95% Statements
...
flowchart TD
    ...
    CLI --> AdapterFactory["Adapter Factory (claude-code | codex | cursor | antigravity | fake)"]
    ...
    InstallPack --> BaseAdapter["Base Adapter (Claude / Codex / Cursor / Antigravity / Fake)"]
    BaseAdapter -->|Spawn CLI / Disposable WS / Cap Memory| AgentCLI["Agent CLI Engine"]
    AgentCLI -->|JSON / JSONL Stream| TranscriptParser["Transcript Parser (claude-code / codex / cursor / antigravity)"]
...
| **Antigravity** | `agy` | `view_file` on `SKILL.md` in `stream-json` | Custom isolated plugin root; throwaway workspace | ✅ Supported, live-tested |
...
| **Path Handling** | Space-safe regex matching | 🟢 **Verified** | All transcript parsers match quoted and bare paths with spaces; symlinked directories resolved. |
...
```
 ...
 ✓ test/transcript/antigravity.test.ts (9 tests)
 ...
 ✓ test/adapters/antigravity.test.ts (6 tests)
