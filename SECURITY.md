...
- **Tools are refused.** Claude Code runs with every tool except `Skill` on the disallowed list, so
  ...
  `--sandbox read-only`. Cursor runs with `--mode ask` (read-only mode).
...
personal plugin skill from answering. Codex and Cursor read skills from your home directory and
offer no override that keeps authentication working, so those reads are surfaced as contamination:
they are never counted as hits, they appear in every report format, and they fail the skill rather
than letting a contaminated run look clean.
