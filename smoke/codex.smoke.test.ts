import { execFileSync } from "node:child_process";
import { mkdirSync, mkdtempSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { describe, expect, it } from "vitest";
import { CodexAdapter } from "../src/adapters/codex.js";

const live = process.env["LIVE"] === "1";

function codexInstalled(): boolean {
  try {
    execFileSync("codex", ["--version"], { stdio: "ignore" });
    return true;
  } catch {
    return false;
  }
}

function haikuPack(): string {
  const packDir = mkdtempSync(join(tmpdir(), "skillcaller-codex-smoke-"));
  mkdirSync(join(packDir, "haiku-writer"), { recursive: true });
  writeFileSync(
    join(packDir, "haiku-writer", "SKILL.md"),
    `---
name: haiku-writer
description: Use when the user asks to write a haiku or any short Japanese-form poem. Triggers on "write a haiku", "compose a haiku", "5-7-5 poem".
---

# Haiku writer

Write a haiku with 5-7-5 syllables. Output only the poem.
`,
  );
  return packDir;
}

describe.skipIf(!live || !codexInstalled())("CodexAdapter (live)", () => {
  it("detects a real skill invocation from the real codex CLI", async () => {
    const outcome = await new CodexAdapter().runPrompt({
      prompt: "write a haiku about gradle builds",
      packDir: haikuPack(),
      timeoutMs: 170_000,
    });

    expect(outcome.usable).toBe(true);
    // When ~/.agents/skills has haiku-writer, Codex reads it from home (reported as contamination).
    const seen = [...outcome.invokedSkills, ...(outcome.foreignSkills ?? [])];
    expect(seen).toContain("haiku-writer");
  });
});
