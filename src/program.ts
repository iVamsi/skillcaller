...
function adapterFor(name: string, scriptPath: string | undefined): AgentAdapter {
  switch (name) {
    ...
    default:
      throw new Error(`unknown agent "${name}"; expected claude-code, codex, cursor or fake`);
  }
}
...
async function runPack(packDir: string, flags: RunFlags): Promise<void> {
...
export function createProgram(): Command {
  ...
  program
    .command("run")
    .argument("<pack>", "directory of skills, each with SKILL.md and evals/triggers.yaml")
    .option("-a, --agent <agent>", "claude-code, codex, cursor or fake", "claude-code")
